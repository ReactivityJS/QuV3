...
test('booting with the REAL repo apps/ directory also loads app-list/user-list/contact-list/profile/forum, and /apps.json lists exactly their (client-bearing) manifests', async () => {
  const base = await mkdtemp(join(tmpdir(), 'qu-relay-'));
  try {
    const relay = await new QuRelay({ storeDir: join(base, 'store'), blobDir: join(base, 'blob'), appsDir: REPO_APPS_DIR, port: 0 }).boot();
    try {
      for (const name of ['app-list', 'user-list', 'contact-list', 'profile', 'forum']) assert.equal(relay.loader.isLoaded(name), true);

      const res = await fetch(`http://localhost:${relay.port}/apps.json`);
      const catalog = await res.json();
      const names = catalog.map((a) => a.name).sort();
      // apps/forum now has a clientMain too (see apps/forum/client.js) -
      // buildAppsCatalog() lists it alongside the others.
      assert.deepEqual(names, ['app-list', 'contact-list', 'forum', 'profile', 'user-list']);
