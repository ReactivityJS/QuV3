...
| `chrome` | `{set: (partial: object) => void}` | The platform-owned sidebar/footer chrome handle (Chrome Inversion) — `container` is already the platform's own content area, so build your UI straight into it and call `chrome.set({primaryAction, navigation, views, settings, fullHeight})` for chrome, instead of calling `@qu/ui`'s `mountAppTemplate()` yourself. Optional to use today — see [`docs/app-navigation-standard.md`](./app-navigation-standard.md) Rule 5a for the full contract and current migration status; `mountAppTemplate()` (Rule 5) stays the recommended entry point for a new app until a second real app has migrated. |
