...
| `chrome` | `{set: (partial: object) => void}` | The platform-owned sidebar/footer chrome handle (Chrome Inversion) — `container` is already the platform's own content area, so build your UI straight into it and call `chrome.set({primaryAction, navigation, views, settings, fullHeight})` for chrome. This is the one supported path — every app in this codebase has migrated onto it, and `@qu/ui`'s `mountAppTemplate()` is no longer called directly by any app (internal machinery now, kept for `chrome.js` itself and every app's own `fakeChrome()` test helper). See [`docs/app-navigation-standard.md`](./app-navigation-standard.md) Rule 5a for the full contract, including the reactive `navigation.list`/`pillTemplate` form for a section backed by real Qu-store data. |
