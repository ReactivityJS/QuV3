# Qu V5 PoC: Space/Node/Field API + Relay Deployment Guide

This is the practical companion to the V5 design discussion (see
`packages/space-core/`, `packages/space-storage/`, `packages/space-transport/`
themselves for the doc comments this guide points at). It covers two things:
...
This is a **separate deployment from the existing `@qu/relay`** (the
repo-root `Dockerfile`/`docker-compose.yml`) — different port (8081 vs
8080), different container, no shared state. Both can run side by side
while V5 stays a PoC alongside the existing V3/V4 platform.
...
- **No auto-reconnect** in `WsClientTransport` — a dropped connection stays
  dropped; reconnect logic (with backoff) is real, separate work (compare
  `packages/sync/src/transports/websocket-client.js`'s existing
  implementation for the shape a V5 version would follow).
...
- **`@qu/space-ui`** (declarative `<qu-view>`/`<qu-bind>`/`<qu-list>`/
  `<qu-text>`-style components) is not built yet — see the V5 plan's own
  status notes for why this was sequenced after the core.
